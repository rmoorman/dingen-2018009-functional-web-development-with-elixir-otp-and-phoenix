defmodule IslandsInterfaceWeb.GameChannel do
  ...
  def handle_in("set_islands", player, socket) do
    ...
    case Game.set_islands(via(socket.topic), player) do
      {:ok, board} ->
        ...
        {:reply, {:ok, %{board: json_friendly(board)}}, socket}
      _ -> {:reply, :error, socket}
    end
  end
  ...
  defp json_friendly(data) when is_list(data) do
    data
    |> Enum.map(&json_friendly/1)
  end

  defp json_friendly(%MapSet{} = v) do
    MapSet.to_list(v)
    |> json_friendly()
  end

  defp json_friendly(%_{} = v) do
    Map.from_struct(v)
    |> json_friendly()
  end

  defp json_friendly(data) when is_map(data) do
    data
    |> Enum.map(fn {k, v} -> {k, json_friendly(v)} end)
    |> Enum.into(%{})
  end

  defp json_friendly(v), do: v
end
